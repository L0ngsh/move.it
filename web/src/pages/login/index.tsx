...
export default function Login({ success, id, token }){
    ...
    async function formSubmit(e: FormEvent) {
        ...
        router.push(`https://github.com/login/oauth/authorize?login=${inputTargetValue}&client_id=${process.env.githubClientId}&scope=user`);
    }
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    const props = {success: false, id: null, token:null };

    if (!code) {
        return { props };
    }

    const githubResponse = await api.post('github', { code });

    const { token } = githubResponse.data;
    
    const loginResponse = await api.post('login', { token });
